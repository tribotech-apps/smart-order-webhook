...
import { HourMinute, Store, StoreCategory } from '../types/Store';
...
export const getStore = async (slug: string) => {
  try {
    ...
    console.log('======================================================*', slug)
    const storeQuery = query(storesRef, where("slug", "==", slug));
    ...
    const store = querySnapshot.docs[0].data() as Store;
    return store;
  } catch (error) {
    // TODO: handle
    return undefined
...
export const getStoreById = async (storeId: string) => {
  try {
    ...
    const store = { _id: storeDoc.id, ...storeDoc.data() } as Store;
...
export const getStoreByWabaPhoneNumberId = async (wabaPhoneNumberId: string): Promise<Store | null> => {
  const storesRef = collection(db, 'Stores'); // Substitua 'Stores' pelo nome correto da sua coleção
  ...
  try {
    ...
    const q = query(storesRef, where('wabaEnvironments.wabaPhoneNumberId', '==', wabaPhoneNumberId));
    const querySnapshot = await getDocs(q);

    // Verificar se algum documento foi encontrado
    if (!querySnapshot.empty) {
      const doc = querySnapshot.docs[0];
      return { ...(doc.data() as Store), _id: doc.id };
    } else {
...
function getStatusByHour(openAt: HourMinute, closeAt: HourMinute) {
  ...
  if (currentHour > closeAt.hour) return 'FECHADA';

  // Current hour earlier than close hours
  if (currentHour < openAt.hour) return 'FECHADA';
...
export function getStoreStatus(storeDb: Store) {
  if (!storeDb) {
    return 'FECHADA';
  }
  ...
  if (openedDate || closedDate) {
    ...
    // Closed Dates Check
    if (closedDate && closedDate?.getDate() === today.getDate()) return 'FECHADA';
...
export const insertStore = async (req: any, res: any): Promise<void> => {
