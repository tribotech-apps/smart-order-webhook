...
import { getSingleProduct, sendProductCard } from './catalogService';
// import { redirectToOrderSummary } from './shoppingService';

export interface GoogleAdddress {
  id: string
  title: string
  description: string
}
...
export const sendAddressConfirmation = async (phoneNumber: string, name: string, address: string,
  wabaEnvironments: WABAEnvironments): Promise<void> => {
  ...
  try {
    ...
    notifyAdmin('Erro ao enviar mensagem:', error.response?.data || error.message);
    // TODO: handle error
    throw (error);
  }
};

export const sendNewAddressMessage = async (phoneNumber: string, wabaEnvironments: WABAEnvironments): Promise<void> => {
...
export const sendWaitForGoogleResponse = async (phoneNumber: string, wabaEnvironments: WABAEnvironments): Promise<void> => {
...
export const sendAddressListResponse = async (phoneNumber: string, addresses: GoogleAdddress[], wabaEnvironments: WABAEnvironments): Promise<void> => {
...
export async function processGooglePredictions(predictions: any[], store: Store): Promise<GoogleAdddress[]> {
...
export async function handleNewAddressFlow(
  from: string,
  text: string,
  ...
  if (!store.wabaEnvironments) {
    notifyAdmin('Erro: store.wabaEnvironments não está definido.');
    const reply = `Desculpe, ocorreu um erro ao processar seu pedido. Por favor, tente novamente.`;
    return;
  }
  ...
  try {
    ...
    notifyAdmin('Erro ao verificar o endereço:', error);
  ...
  // console.log('Fluxo atualizado para ADDRESS_CONFIRMATION');
  await sendWaitForGoogleResponse(from, store.wabaEnvironments)
  ...
  try {
    ...
    const predictions = await Promise.all(
      response.data.predictions.slice(0, 9).map(async (prediction) => {
    ...
    try {
      ...
      await sendAddressListResponse(from, predictions, store.wabaEnvironments);

      // console.log('Fluxo atualizado para NEW_ADDRESS');
    }
    catch (error) {
      notifyAdmin('Erro ao processar a solicitação2:', error);
    }


  } catch (error) {
    notifyAdmin('Erro ao processar a solicitação1:', error);
    res.status(500).send('Erro ao processar a solicitação3.');
  }

}

export async function handleUseAddress(
  from: string,
  currentConversation: Conversation,
  currentUser: any,
  store: Store,
  ...
  if ((currentConversation?.flow as ConversationFlow) !== 'ADDRESS_CONFIRMATION') {
  ...
  if (currentConversation.previousFlow === 'Fazer um Pedido') {
    try {
      ...
      notifyAdmin('Erro ao processar o botão "Usar este endereço":', error);
    }
  ...
  if (currentConversation.previousFlow === store.singleProductText) {
    try {
      ...
      if (!currentConversation.store?.menu?.length) {
        ...
        notifyAdmin(`Não há produtos disponíveis no menu da loja ${store.name}`)
        return;
      }

      const produtct = getSingleProduct(currentConversation.store?.menu)
      if (!produtct?.length) {
        // envia mensagem que nao existe produto
        const reply = `Desculpe, não há produtos disponíveis no momento. Por favor, tente novamente mais tarde.`;
        await sendMessage({
      ...
      await sendProductCard(from, produtct[0], store.wabaEnvironments);
...
export async function handleInformOtherAddress(
  from: string,
  currentConversation: Conversation
): Promise<void> {
  ...
  // Verificar se o fluxo atual é "ADDRESS_CONFIRMATION"
  if ((currentConversation?.flow as ConversationFlow) !== 'ADDRESS_CONFIRMATION' &&
    (currentConversation?.flow as ConversationFlow) !== 'EDIT_OR_REMOVE_SELECTION') {
  ...
  try {
    ...
    notifyAdmin('Erro ao processar o botão "Informar outro endereço":', error);
  }
}
