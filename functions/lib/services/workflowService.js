...
class WorkflowService {
     ...
     */
    async findOrderByIdProperty(orderId) {
        try {
     ...
     */
    async moveOrderToNextFlow(transition) {
        ...
        try {
            const result = await (0, firestore_1.runTransaction)(firebase_1.db, async (transaction) => {
                ...
                const updates = {
                    ...
                    workflow: (0, firestore_1.arrayUnion)({
                        flowId: fromFlowId,
                        minutes: minutes,
                        completedAt: now
                    }),
     ...
     */
    async sendWorkflowMessages(orderId, flowId, orderData, storeId) {
        try {
            ...
            if (orderData.phoneNumber && storeId) {
                ...
                try {
                    ...
                    if (store?.wabaEnvironments) {
                        ...
                        promises.push(this.sendWhatsAppMessage(flowId, orderData, store).catch(error => {
            ...
            promises.push(this.sendPushNotification(flowId, orderId, storeId).catch(error => {
     ...
     */
    async sendCustomerWhatsAppUpdate(orderData, fromFlowId, toFlowId, cancel, storeId) {
        try {
            ...
            const store = await (0, storeController_1.getStoreById)(storeId);
     ...
     */
    async sendWhatsAppMessage(flowId, orderData, store) {
        try {
            ...
            const template = messageTemplates[flowId];
            if (!template) {
     ...
     */
    async sendPushNotification(flowId, orderId, storeId) {
        try {
            ...
            const template = notificationTemplates[flowId];
            if (!template) {
     ...
     * Cancela um pedido
     */
    async cancelOrder(orderId, reason, storeId) {
        try {
     ...
     */
    async sendCancellationMessages(orderData, reason) {
