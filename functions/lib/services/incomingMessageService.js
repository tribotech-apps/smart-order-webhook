                    ...
                    if (selectedAddress) {
                        ...
                        if (distance > store.deliveryMaxRadiusKm || 0) {
                            ...
                            await (0, messagingService_1.sendMessage)({
                                ...
                                to: "+" + from,
                                type: 'text',
                                text: { body: `O endereço informado está fora do nosso raio de entrega. Fazemos entrega em um raio de ${store.deliveryMaxRadiusKm} metros.` }
                            }, store.wabaEnvironments);
                            return;
                        }
                    }
        ...
        try {
            ...
            if (typeof content === 'object') {
                switch (content.action) {
                    // case 'Informando Endereco': // REMOVIDO - webhook gerencia endereços
                    case 'DISABLED_Informando_Endereco':
                        console.log('Cliente informando endereço:', content);
                        // Buscar dados para validação de endereço
                        const enderecoInformado = content.endereco || content.message;
                        console.log('Endereço capturado:', enderecoInformado);
                        // Atualiza a Conversation com a mensagem d 
                        await (0, conversationController_1.updateConversation)(currentConversation, {
                            flow: 'ADDRESS_CONFIRMATION'
                        });
                        try {
                            ...
                            const response = await clientGoogle.placeAutocomplete({
                                params: {
                                    input: `${enderecoInformado} ${currentConversation?.store?.address?.city || ''} ${currentConversation?.store?.address?.state || ''}`,
                                    types: google_maps_services_js_1.PlaceAutocompleteType.geocode,
                                    key: GOOGLE_PLACES_API_KEY,
                                },
                            });
                            ...
                            // Encontrou 1 ou mais endereços - passar controle para a IA
                            // A IA vai conduzir a conversa até o cliente confirmar o endereço
                            // Salvar predictions no historyItems para a IA usar
                            await (0, conversationController_1.updateConversation)(currentConversation, {
                                flow: 'ADDRESS_CONFIRMATION',
                                historyItems: predictions
                            });
                            // Formatar predictions para enviar à IA
                            const predictionsText = predictions.map((pred, index) => `${index + 1}. ${pred.description}`).join('\n');
                            // Adicionar opção "não está na lista"
                            const fullPredictionsText = predictionsText + `\n${predictions.length + 1}. Endereço não está na lista - informar outro endereço`;
                            // Preparar contexto para a IA
                            const addressConfirmationPrompt = `O cliente informou o endereço "${enderecoInformado}". Encontrei ${predictions.length} endereços correspondentes:\n\n${fullPredictionsText}\n\nPergunte ao cliente qual endereço é o correto digitando o número correspondente.`;
                            // Chamar IA novamente passando os resultados encontrados
                            const aiResponse = await classifyUserMessage(addressConfirmationPrompt, store, currentConversation.history + ' --- ' + addressConfirmationPrompt);
                            const aiContent = parseAIResponse(aiResponse.message?.content);
                            // Enviar resposta da IA para o cliente
                            await (0, messagingService_1.sendMessage)({
                                messaging_product: 'whatsapp',
                                to: "+" + from,
                                type: 'text',
                                text: { body: aiContent.message }
                            }, store.wabaEnvironments);
                            console.log('✅ Controle passado para IA com predictions:', predictions);
                        }
                        catch (error) {
                            (0, messagingService_1.notifyAdmin)('Erro ao consultar Google Places:', error);
                            await (0, messagingService_1.sendMessage)({
                                ...
                                to: "+" + from,
                                type: 'text',
                                text: { body: '❌ Erro ao buscar endereço. Por favor, informe seu endereço completo.\n\n**Formato:** Rua, Número, Bairro, Cidade\n**Exemplo:** Rua das Flores, 123, Centro, São Paulo' }
                            }, store.wabaEnvironments);
                        }
                        return;
                    case 'Endereco Confirmado':
                        console.log('IA confirmou endereço:', content);
                        // Extrair dados do endereço confirmado
                        const enderecoConfirmado = content.endereco || content.message;
                        console.log('Endereço confirmado pela IA:', enderecoConfirmado);
                        // Recuperar predictions salvas na conversation
                        const savedPredictions = currentConversation.historyItems || [];
                        let selectedAddress = null;
                        // Tentar identificar qual endereço foi selecionado
                        for (const pred of savedPredictions) {
                            if (enderecoConfirmado.includes(pred.description) ||
                                enderecoConfirmado.includes(pred.title)) {
                                // Buscar no cache o endereço completo
                                selectedAddress = addressCache[pred.id];
                                break;
                            }
                        }
                        if (!selectedAddress) {
                            console.error('Endereço selecionado não encontrado no cache');
                            await (0, messagingService_1.sendMessage)({
                                messaging_product: 'whatsapp',
                                to: "+" + from,
                                type: 'text',
                                text: { body: '❌ Erro ao processar endereço. Vamos tentar novamente.' }
                            }, store.wabaEnvironments);
                            return;
                        }
                        // Validar raio de entrega
                        if (!store.address?.lat || !store.address?.lng) {
                            console.error('Store não tem coordenadas configuradas');
                            await (0, messagingService_1.sendMessage)({
                                messaging_product: 'whatsapp',
                                to: "+" + from,
                                type: 'text',
                                text: { body: '❌ Erro interno: configuração de entrega não encontrada.' }
                            }, store.wabaEnvironments);
                            return;
                        }
                        const distance = calculateDistance(store.address.lat, store.address.lng, selectedAddress.lat, selectedAddress.lng);
                        const isWithinRadius = distance <= (store.deliveryMaxRadiusKm || 10);
                        if (!isWithinRadius) {
                            await (0, messagingService_1.sendMessage)({
                                messaging_product: 'whatsapp',
                                to: "+" + from,
                                type: 'text',
                                text: {
                                    body: `❌ Infelizmente não atendemos nesta região.\n\n📍 **Endereço:** ${selectedAddress.description}\n📏 **Distância:** ${distance.toFixed(2)} km\n🚛 **Raio máximo:** ${store.deliveryMaxRadiusKm || 10} km`
                                }
                            }, store.wabaEnvironments);
                            // Resetar para pedir novo endereço
                            await (0, conversationController_1.updateConversation)(currentConversation, {
                                flow: 'WELCOME',
                                historyItems: []
                            });
                        ...
                        // Endereço validado - salvar e continuar
                        console.log('✅ Endereço confirmado e validado');
                        await (0, conversationController_1.updateConversation)(currentConversation, {
                        ...
                        // TODO: Salvar no usuário se logado
                        if (user?.user?.uid) {
                            console.log('TODO: Salvar endereço no usuário');
                        }
                        return;
