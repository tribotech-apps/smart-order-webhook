...
exports.OrderAlertScheduler = void 0;
...
class OrderAlertScheduler {
    /**
     * Agenda alertas usando Cloud Tasks no horário específico
     */
    static async scheduleStageAlerts(orderId, stageId, storeId, createdAt, stageLimitMinutes) {
        ...
        try {
            ...
            const created = luxon_1.DateTime.fromJSDate(createdAt).setZone('America/Sao_Paulo');
     ...
     */
    static async createTask({ taskId, scheduleTime, payload }) {
     ...
     */
    static async cancelOrderTasks(orderId) {
        try {
            const parent = tasksClient.queuePath(project, location, queue);
     ...
     */
    static async handleStageChange(orderId, newStageId, storeId, createdAt) {
        ...
        try {
            ...
            switch (newStageId) {
                case 1:
                    limitMinutes = store.rowTime;
                    break;
                case 2:
                    limitMinutes = store.rowTime + store.productionTime;
                    break;
                case 3:
                    limitMinutes = store.rowTime + store.productionTime + store.deliveryTime;
                    break;
                default:
     ...
     */
    static async sendWarningAlert(orderId, stageId, storeId) {
     ...
     */
    static async sendOverdueAlert(orderId, stageId, storeId) {
    ...
    static async updateOrderStatus(orderId, status) {
        try {
            ...
            const orderRef = (0, firestore_1.doc)(firebase_1.db, 'Orders', order._id);
            await (0, firestore_1.updateDoc)(orderRef, { alertStatus: status });
    ...
    static async getOrder(orderId) {
        try {
            ...
            const q = (0, firestore_1.query)(ordersCollection, (0, firestore_1.where)('id', '==', orderId), (0, firestore_1.limit)(1));
            ...
            return { _id: orderDoc.id, ...orderDoc.data() };
        }
        catch (error) {
     ...
     */
    static async sendStageChangeNotificationToCustomer(orderId, newStageId, storeId) {
     ...
     */
    static async sendConfirmationRequestToStore(orderId, storeId) {
        ...
        try {
            ...
            const itemsSummary = order.items?.map(item => `• ${item.quantity}x ${item.menuName}${item.price ? ` - R$ ${item.price.toFixed(2)}` : ''}`).join('\n') || 'Itens não especificados';
...
exports.OrderAlertScheduler = OrderAlertScheduler;
