...
// // console.log('WABA_ACCESS_TOKEN:', WABA_ACCESS_TOKEN);
...
router.post('/webhook', async (req, res) => {
    ...
    if (!req.body.entry?.length) {
        // Se não houver "entry", retorna um erro 400
    ...
    if (!phoneNumberId) {
        // Se não houver phoneNumberId, retorna um erro 400
    ...
    if (req.body.object === 'whatsapp_business_account') {
            ...
            req.body.entry.forEach((entry) => {
                entry.changes.forEach(async (change) => {
                    ...
                    if (value.messages) {
                        ...
                        switch (type) {
                            case 'text':
                                ...
                                if (currentConversation.flow === 'CATEGORIES') {
                                    // Enviar a lista de categorias para o usuário
                                    await (0, catalogService_1.sendCategoriesMessage)(from, store.categories, store.menu, store.wabaEnvironments);
                                    return;
                                }
                            ...
                            case 'interactive':
                                    ...
                                    switch (buttonReply) {
                                        ...
                                        case 'Trocar categoria':
                                            ...
                                            if (currentConversation.flow !== 'PRODUCTS')
                                                return;
                                            await (0, catalogService_1.sendCategoriesMessage)(from, store.categories, store.menu, store.wabaEnvironments); // Enviar lista de categorias
                                            break;
                                        case 'Fazer Nova Compra':
                                            ...
                                            try {
                                                ...
                                                (0, messagingService_1.sendWelcomeMessage)(from, '', store.wabaEnvironments, store, store.logo);
                                                return;
                                            }
                                            catch (error) {
                                        ...
                                        default:
                                            ...
                                            if (typeof buttonReplyId === 'string' && buttonReplyId.startsWith('more_')) {
                                                ...
                                                await (0, catalogService_1.sendProductsWithPagination)(from, products, nextPage, store.wabaEnvironments);
                                ...
                                if (interactiveType === 'list_reply') {
                                    ...
                                    if (currentConversation?.flow === 'CATEGORIES' || currentConversation?.flow === 'PRODUCTS') {
                                        ...
                                        if (products.length > 0) {
                                            ...
                                            await (0, messagingService_1.sendMessage)({
                                                ...
                                                type: 'text',
                                                text: {
                                                    body: `Aqui estão os produtos da categoria "${selectedCategory.categoryName}". Você pode visualizar os produtos em grupos de 5. Para ver a próxima página, clique no botão "Ver mais".`,
                                                },
                                            }, store.wabaEnvironments);
                                            // Enviar os primeiros 5 produtos
                                            await (0, catalogService_1.sendProductsWithPagination)(from, products, 1, store.wabaEnvironments);
                                        }
                                        else {
                                    ...
                                    if (currentConversation.flow === 'EDIT_ITEM_QUANTITY') {
                                        ...
                                        // Extrair a quantidade do ID selecionado (ex.: "quantity_3" -> 3)
                                        const quantity = parseInt(listReplyId.replace('quantity_', ''), 10);
                                        console.log('PREVISOUS FLOW', currentConversation.previousFlow);
                                        if (isNaN(quantity) || quantity < 1 || quantity > 10) {
                                            (0, messagingService_1.notifyAdmin)('Quantidade inválida selecionada:', listReplyId);
                                            await (0, messagingService_1.sendMessage)({
                                                messaging_product: 'whatsapp',
                        ...
                        // console.warn('Nenhuma mensagem encontrada no evento.');
                    }
                });
            });
        ...
        res.status(404).send('Evento não suportado');
    }
});
